     ...
     for q in range(0,i):
      if (exams[q]['Результатов'] == new_exams[q]['Результатов']):
        co = co + 1
          ...
          counter = 1
          bot.send_message("Обновление результатов по %s : Было результатов: %s | Стало результатов: %s" % (new_exams[q]['Экзамен'],str(exams[q]['Результатов']),str(new_exams[q]['Результатов'])))
     if counter == 1:
         main()
         counter = 0
     bot.timer()
